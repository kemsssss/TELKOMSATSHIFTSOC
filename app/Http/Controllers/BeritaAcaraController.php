...
class BeritaAcaraController extends Controller
{
    ...
    public function cetakPDF(Request $request) {
        ...
        $lama_ttd = $lastPetugasLama && $lastPetugasLama->ttd_path ? $this->getBase64FromStorage($lastPetugasLama->ttd_path) : null;
        $baru_ttd = $lastPetugasBaru && $lastPetugasBaru->ttd_path ? $this->getBase64FromStorage($lastPetugasBaru->ttd_path) : null;
    ...
    public function print($id)
    {
        ...
        $data = [
            ...
            'lama_ttd'       => $this->getBase64FromStorage($lastPetugasLama->ttd_path ?? null),
            'baru_ttd'       => $this->getBase64FromStorage($lastPetugasBaru->ttd_path ?? null),
            'lama_nama'      => $lastPetugasLama->nama ?? '-',
            'lama_nik'       => $lastPetugasLama->nik ?? '-',
            'baru_nama'      => $lastPetugasBaru->nama ?? '-',
            'baru_nik'       => $lastPetugasBaru->nik ?? '-',
